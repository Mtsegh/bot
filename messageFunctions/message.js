...
const handle_message = async (bot, msge) => {
    ...
    if (/\/start/.test(message)) {
    ...
    } else if (state.isAUT && message.length<20) {
        try {
            ...
            if (isuser) {
                ...
                if (state.cpass) {
                    ...
                    await sendMessage(bot, chatId, 'Logging in...').then((msg) => {
                        accountswitch(chatId, state.aut).then(async(login) => {
        ...
        await editMessageText(bot, 'Enter passcode to process your request', {
            chat_id: chatId,
            message_id: state.msgId,
        })
...
async function sendStartMessage(bot, chatId, register) {
    ...
    await sendMessage(bot, chatId, `Hello Dear. We are happy to have you on our Vendor Bot. Here is your Account Unique Token *${register.AUT}*. You will need this in case you want to access your VB account on another telegram account, so keep it safe dear, this token contains all your  account info and transactions. Select an option below to proceed:`, options)
        .then(async() => {
...
async function sendMainMenu(bot, chatId, msgId) {
    ...
    try {
        ...
        if (error.response) {
            console.log(`Message with ID ${msgId} not found or deleted.`);
            // Optionally, you can send a new message if editing fails
            await sendMessage(bot, chatId, 'Choose an option:', options);
        } else {
    ...
    const dataForms = {
        reply_markup: JSON.stringify({
            inline_keyboard: [
                ...
                [{ text: '9mobile', callback_data: 'mobile9' }],
